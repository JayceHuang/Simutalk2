...
public class PayDialogListener implements RenrenDialogListener {
	...
	private void DoWithError(String url) {
		...
		if (value.containsKey("code")) {
			error = new RenrenError(Integer.parseInt(value.getString("code")), value.getString("description"), null);
		}
		/**
		 * 解析并本地存储
		 */
	...
	private void DoWithLocalError(String url) {
		...
		Bundle value = PayUtil.parseUrl(url);
		if (value.containsKey("code")) {
			error = new RenrenError(RenrenError.ERROR_CODE_UNKNOWN_ERROR, value.getString("code"), null);
		}
		/**
		 * 解析并本地存储
		 */
	...
	private PayOrder generateAndStorePayOrder(Bundle value) {
		...
		if (value.containsKey("orderedTime")) {
			try {
				order.setDealTime(new Date(Long.parseLong(value.getString("orderedTime"))));
		...
		if (value.containsKey("payStatusCode")) {
			order.setPayStatusCode(Integer.parseInt(value.getString("payStatusCode")));
		}
	...
	public int onPageBegin(String url) {
		...
		if (url != null && url.startsWith(PayUtil.PAY_URL_PREFIX)) {
			...
			// 出错的情况
			else if (url.startsWith(PayUtil.PAY_ERROR_URL_PREFIX)
					|| url.startsWith(PayUtil.PAY_FIXORDER_ERROR_URL_PREFIX)) {
				DoWithError(url);
			} else {
				Util.logger("new PayError()");
				RenrenError error = new RenrenError(RenrenError.ERROR_CODE_UNKNOWN_ERROR,
						"PAY_FIXORDER_ERROR_URL_PREFIX", "");
				if (null != mListener)
					if (mListener instanceof IPayListener) {
						((IPayListener) mListener).onError(error);
					} else if (mListener instanceof IPayRepairListener) {
