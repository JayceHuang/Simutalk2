...
public class PayRepaiListAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, final ViewGroup parent) {
		if (items != null && position >= 0 && position < items.size()) {
			View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.renren_sdk_pay_repair_item, null);
			final RepairListItem item = items.get(position);

			TextView orderNumView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_order_num);
			TextView desView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_des);
			TextView bidView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_bid);
			TextView timeView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_time);
			TextView amountView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_amount);
			TextView statusView = (TextView) itemView.findViewById(R.id.renren_sdk_pay_repair_item_status);
			Button repairBtn = (Button) itemView.findViewById(R.id.renren_sdk_pay_repair_item_repair_button);
			...
			repairBtn.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					try {
						RenrenPay.getInstance().repairOrder(parent.getContext(), item.getOrder());
					} catch (Exception e) {
