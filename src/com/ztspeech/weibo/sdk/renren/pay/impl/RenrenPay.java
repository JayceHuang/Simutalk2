...
import com.ztspeech.weibo.sdk.renren.pay.IPayListener;
...
public class RenrenPay implements IRenrenPay {
	...
	private boolean asynCheckApp() {
		...
		params.putString("app_encode", PayUtil.getAppCode(mRenren.getAppId(), "", mRenren.getSecret(), time));
	...
	public List<PayOrder> getStoredPayOrders(Context c) {
		...
		return payStoreHelper.getPayOrder(Integer.parseInt(mRenren.getAppId()), (int) mRenren.getCurrentUid());
	}
	...
	private RenrenPay(Context context, Renren renren, IPayRepairListener listener) {
	...
	private boolean pay(final Activity act, final Payment payment) {
		...
		RenrenAuthListener authListener = new RenrenAuthListener() {
			...
			public void onRenrenAuthError(RenrenAuthError renrenAuthError) {
				l.onError(new RenrenError(RenrenError.ERROR_CODE_AUTH_FAILED, renrenAuthError.getErrorDescription(),
						renrenAuthError.getErrorUri()));
			}

			@Override
			public void onCancelLogin() {
				l.onError(new RenrenError(RenrenError.ERROR_CODE_AUTH_CANCELLED, "user_cancel", null));
			}

			@Override
			public void onCancelAuth(Bundle values) {
				l.onError(new RenrenError(RenrenError.ERROR_CODE_AUTH_CANCELLED, "user_cancel_auth", null));
			}
		};
	...
	private void openDialogForPay(Context context, Payment payment) {
		if (!this.is4Test && this.isInitalized && mAppState.getStatusCode() != AppState.OK) {
		 ...
		 * 生成PayOrder 对象
		 */
		PayOrder payOrder = new PayOrder(mRenren.getAppId(), mRenren.getCurrentUid(), payment);
		String url = null;
		if (!this.is4Test) {
		...
		String data = PayUtil.generateOrderDatas(mRenren.getAppId(), mRenren.getSecret(), mRenren.getAccessToken(),
				payment.getOrderNumber(), payment.getAmount(), payment.getDescription(), payment.getPayment());
		RenrenDialog dialog = new RenrenDialog(context, url, data, new PayDialogListener(payOrder,
				payment.getPayListener()));
	 ...
	 * @param context
	 */
	private void openDialogForRepair(Context context, PayOrder payOrder, IPayRepairListener listener) {
		...
		String data = PayUtil.generateRepairDatas(payOrder.getBid(), payOrder.getAppId(), payOrder.getOrderNumber(),
				payOrder.getAmount(), mRenren.getSecret(), mRenren.getAccessToken(), payOrder.getUserId(), payOrder
						.getOrderTime().getTime());
		RenrenDialog dialog = new RenrenDialog(context, fixOrderUrl, data, new PayDialogListener(payOrder, listener));
		dialog.show();
		payStoreHelper.addOrUpdatePay(payOrder);
	}
	 ...
	 * @return
	 */
	public static synchronized IRenrenPay getInstance(Context c, Renren r, IPayRepairListener l) {
		if (null == instance) {
	...
	public void removeOrderByOrderNumber(String orderNumber) {
		...
		if (null != orderNumber && null != payStoreHelper) {
			payStoreHelper.removeByUidAndOrderNumber((int) this.mRenren.getCurrentUid(), orderNumber);
		}
	}
