...
public class FeedPublishRequestParam extends RequestParam implements Parcelable {
	...
	private int checkFeed() {
		...
		if (description != null && description.length() > FeedPublishRequestParam.DESCRIPTION_MAX_LENGTH) {
		...
		if (actionName != null && actionName.length() > FeedPublishRequestParam.ACTION_NAME_MAX_LENGTH) {
	...
	private int checkWidgetFeed() {
		...
		if (description != null && description.length() > FeedPublishRequestParam.WIDGET_DESCRIPTION_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.DESCRIPTION_TOO_LONG;
		}
	...
	public Bundle getParams() throws RenrenException {
		if (name == null || name.length() == 0 || description == null || description.length() == 0 || url == null
				|| url.length() == 0) {
		...
		if (checkFeed() != NORMAL_LENGTH) {
			...
			throw new RenrenException(RenrenError.ERROR_CODE_ILLEGAL_PARAMETER, errorMsg, errorMsg);
		}
