 ...
 */
package com.ztspeech.weibo.sdk.renren.feed;
...
public class FeedHelper {
	...
	public FeedPublishResponseBean publish(FeedPublishRequestParam feed) throws RenrenException, Throwable {
		...
		if (rrError != null) {
			...
			try {
				...
				throw new RenrenException(RenrenError.ERROR_CODE_UNABLE_PARSE_RESPONSE, je.getMessage(),
						je.getMessage());
			}
		}
	}
	 ...
	 *            若超出了长度，是否自动截短至限制的长度
	 */
	public void asyncPublish(Executor pool, final FeedPublishRequestParam feed,
			final AbstractRequestListener<FeedPublishResponseBean> listener, final boolean truncOption) {
		pool.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					Util.logger(rre.getMessage());
					if (listener != null) {
						listener.onRenrenError(new RenrenError(rre.getErrorCode(), rre.getMessage(), rre
								.getOrgResponse()));
					}
	...
	public void startFeedPublishWidget(String appID, Activity activity, FeedPublishRequestParam feed,
		...
		/*
		 * 使用widgetDialog发布状态跟权限授予没有关系，此处不必要做权限限制。 fixed:shaopeng.han
		 */
		// if (!renren.isSessionKeyValid()) {
		// String errorMsg = "Session key is not valid.";
		...
		renren.widgetDialog(activity, params, new RenrenWidgetListener() {
			...
			public void onError(Bundle retParams) {
				if (listener != null) {
					listener.onRenrenError(new RenrenError(retParams.getString("error")
			...
			public void onComplete(Bundle retParams) {
				if (listener != null) {
					if (retParams.containsKey(FeedPublishResponseBean.RESPONSE)) {
						listener.onComplete(new FeedPublishResponseBean(retParams
								.getString(FeedPublishResponseBean.RESPONSE)));
					} else {
			...
			public void onCancel(Bundle retParams) {
				if (listener != null) {
					...
					RenrenError error = new RenrenError(RenrenError.ERROR_CODE_OPERATION_CANCELLED, errorMsg, errorMsg);
