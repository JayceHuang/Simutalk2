...
public class CreateAlbumActivity extends AbstractRenrenRequestActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		AuthorizationHelper.check(renren, CreateAlbumActivity.this,
				new String[] { PhotoHelper.CREATE_ALBUM_PERMISSION }, new RenrenAuthListener() {
	...
	private void initComponents() {
		...
		for (int i = 0; i < albumPrivacyTypes.length; i++) {
			if (albumPrivacyTypes[i].getOunces() != AlbumPrivacyType.PASSWORD.getOunces()) {
				spinnerValues.add(albumPrivacyTypes[i]);
			}
		}

		albumVisibleValue = (Spinner) findViewById(R.id.renren_sdk_album_visible_value);
		ArrayAdapter<AlbumPrivacyType> adapter = new ArrayAdapter<AlbumPrivacyType>(this,
				android.R.layout.simple_spinner_item, spinnerValues);
		adapter.setDropDownViewResource(android.R.layout.simple_dropdown_item_1line);
		albumVisibleValue.setAdapter(adapter);

		albumVisibleValue.setOnItemSelectedListener(new Spinner.OnItemSelectedListener() {
		...
		submit.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (name == null || TextUtils.getTrimmedLength(name) == 0) {
					...
					// 调用接口完成创建相册的工作
					new AsyncRenren(renren).createAlbum(albumRequest,
							new AbstractRequestListener<AlbumCreateResponseBean>() {
								...
								public void onFault(Throwable fault) {
									if (fault != null) {
										...
										Util.logger("exception in creating album!: " + fault.getMessage());
									}
								}

								@Override
								public void onComplete(AlbumCreateResponseBean albumResponse) {
