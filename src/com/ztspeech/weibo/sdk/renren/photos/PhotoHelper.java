...
public class PhotoHelper {
	 ...
	 *            照片描述，非必须参数，可为空
	 */
	public void startUploadPhotoActivity(Activity activity, File file, String caption) {
	...
	public AlbumCreateResponseBean createAlbum(AlbumCreateRequestParam albumRequest) throws RenrenException, Throwable {
		...
		if (!renren.isSessionKeyValid()) {
			...
			throw new RenrenException(RenrenError.ERROR_CODE_TOKEN_ERROR, "没有登录", "没有登录");
		}
		...
		// 用请求结果串构造返回对象
		AlbumCreateResponseBean albumResponse = new AlbumCreateResponseBean(result);
		...
		pool.execute(new Runnable() {
			...
			public void run() {
				try {
					...
					if (listener != null) {
						listener.onRenrenError(new RenrenError(e.getErrorCode(), e.getMessage(), e.getOrgResponse()));
					}
				} catch (Throwable e) {
	 ...
	 *             ,Throwable
	 */
	public AlbumGetResponseBean getAlbums(AlbumGetRequestParam albumsRequest) throws RenrenException, Throwable {
		// 同步数据接口只判断用户是否登录，不提供自动跳转到验证界面验证的功能
		if (!renren.isSessionKeyValid()) {
			// 若用户没有登录，则直接抛出异常
	 ...
	 *             , Throwable
	 */
	public PhotoUploadResponseBean uploadPhoto(PhotoUploadRequestParam photoRequest) throws RenrenException, Throwable {
		// 同步数据接口只判断用户是否登录，不提供自动跳转到验证界面验证的功能
		if (!renren.isSessionKeyValid()) {
			// 若用户没有登录，则直接抛出异常
		...
		if (photoRequest.getFile() == null) {
			...
			throw new RenrenException(RenrenError.ERROR_CODE_NULL_PARAMETER, "上传失败，没有文件！", "上传失败，没有文件！");
		}
		...
		if (!fileName.endsWith(".jpg") && !fileName.endsWith(".jpeg") && !fileName.endsWith(".png")
			...
			throw new RenrenException(RenrenError.ERROR_CODE_ILLEGAL_PARAMETER, "暂不支持此格式照片，请重新选择", "暂不支持此格式照片，请重新选择");
		}
		...
		if (content == null) {
			...
			throw new RenrenException(RenrenError.ERROR_CODE_NULL_PARAMETER, "上传失败，文件内容为空！", "上传失败，文件内容为空！");
		}
		...
		String result = null;
		try {
			result = renren.publishPhoto(photoRequest.getAid(), content, photoRequest.getFile().getName(),
		...
		// 用请求结果构造返回实体
		PhotoUploadResponseBean photoResponse = new PhotoUploadResponseBean(result);
