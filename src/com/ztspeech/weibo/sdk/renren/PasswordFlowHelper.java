...
public class PasswordFlowHelper {
	 ...
	 * @throws Throwable
	 */
	public PasswordFlowResponseBean login(PasswordFlowRequestParam param) throws RenrenException, Throwable {
	 ...
	 *            Renren对象
	 */
	public void login(Executor pool, final PasswordFlowRequestParam param, final RenrenAuthListener listener,
			final Renren renren) {
			...
			public void run() {
				...
				try {
					...
					if (passwordFlow != null) {
						...
						Util.logger("null response");
						throw new RenrenException(RenrenError.ERROR_CODE_UNKNOWN_ERROR, "null response",
								"null response");
					}
				} catch (RenrenException e) {
					Util.logger("renren exception " + e.getMessage());
					if (listener != null) {
						listener.onRenrenAuthError(new RenrenAuthError(String.valueOf(e.getErrorCode()), e
								.getOrgResponse(), PASSWORD_FLOW_URL));
					}
					e.printStackTrace();
				} catch (Throwable e) {
	 ...
	 *            Renren对象
	 */
	public void login(final Activity activity, PasswordFlowRequestParam param, final RenrenAuthListener listener,
			final Renren renren) {
	...
	class LoginEntryDialog extends Dialog {
		...
		public LoginEntryDialog(Activity activity, PasswordFlowRequestParam param, RenrenAuthListener listener,
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			final LinearLayout loginEntryView = (LinearLayout) getLayoutInflater().inflate(
					R.layout.renren_sdk_login_entry, null);
		...
		private void initViews(LinearLayout loginEntryView) {
			final EditText userNameEditText = (EditText) loginEntryView
					.findViewById(R.id.renren_sdk_login_entry_username);
			final EditText passwordEditText = (EditText) loginEntryView
					.findViewById(R.id.renren_sdk_login_entry_password);
