...
public class SSO {
	...
	private boolean tryRRClientAuth(Intent intent) {
		...
		intent.setClassName("com.renren.mobile.android", "com.renren.mobile.android.auth.SSOAuthActivity");
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				if (error != null) {
					if (error.equals("service_disabled")) {
						Log.d(LOG_TAG, "Hosted auth currently " + "disabled. Retrying dialog auth...");
						...
						mAuthListener.onRenrenAuthError(new RenrenAuthError(error, error_description, null));
					}
				} else {// No errors.
				...
				if (data != null) {
					...
					mAuthListener.onRenrenAuthError(new RenrenAuthError(data.getStringExtra("error"), data
							.getStringExtra("error_description"), data.getStringExtra("failing_url")));
	...
	public void widgetDialog(Activity activity, Bundle params, final RenrenWidgetListener listener, String url,
			int requestCode) {
		Intent intent = new Intent();
		intent.putExtra("client_id", this.renren.getApiKey());
	...
	private boolean tryRRClientDialog(Intent intent) {
		intent.setClassName("com.renren.mobile.android", "com.renren.mobile.android.auth.SSODialog");
		if (!validateAppSignatureForIntent(this.mActivity, intent, RR_APP_SIGNATURE)) {
			return false;
		}
	...
	private boolean validateAppSignatureForIntent(Activity activity, Intent intent, String targetSignatures) {
	...
	private boolean startActivity(Intent intent) {
		try {
			...
			e.printStackTrace();
			Log.w(LOG_TAG, "start activity " + intent.getComponent().getClassName() + " error " + e.getMessage());
		}
		return false;
	}
